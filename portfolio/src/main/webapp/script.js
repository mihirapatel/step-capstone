...
function dialogflowTheBlob(blob) {
  ...
  formData.append('test-file', new File(["hello test stuff"], "filename"))
  ...
  fetch('/audio-input', {
    ...
    container.value += stream; 
  });
//   fetch('/audio-input').then(response => response.text()).then((stream) => {
//     var container = document.getElementsByName("streamed-stuff")[0];
//     container.value += stream;
//   });
}
